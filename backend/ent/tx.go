...
func (tx *Tx) init() {
	tx.Organ = NewOrganClient(tx.config)
	tx.Physician = NewPhysicianClient(tx.config)
	tx.Spaciality = NewSpacialityClient(tx.config)
	tx.TypeDisease = NewTypeDiseaseClient(tx.config)
}
...
// applies a query, for example: Organ.QueryXXX(), the query will be executed
